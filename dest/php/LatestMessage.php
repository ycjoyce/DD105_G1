...
$sql= "select * from message m join blacklist b on m.sendmemno=b.memno or m.getmemno=b.memno where msgNo in (select max(msgNo) from message where getmemno='{$_SESSION["memNo"]}' or sendmemno='{$_SESSION["memNo"]}' group by concat(if(sendmemno='{$_SESSION["memNo"]}',sendmemno,getmemno),',',if(getmemno='{$_SESSION["memNo"]}',sendmemno,getmemno)) ) and m.sendmemno<>b.blackmemno and m.getmemno<>b.blackmemno order by msgNo;";
$msg= $pdo->query($sql);
